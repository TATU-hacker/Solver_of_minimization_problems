...
class MainWindow(Ui_MainWin):
    ...
    def setupUi(self, mainWindow):
        ...
        self.functionBox.setPlaceholderText("ex: pow(x1,2)+log(xyx, e)")
        self.__connectSlots()
    ...
    def __nextButtonClicked(self):
        ...
        if bandwidthDialog.exec() == QDialog.Accepted:
            ...
            try:
                ...
                self.nextButton.setDisabled(True)
                self.label_function_error.setText(QtCore.QCoreApplication.translate("MainWin", 'Cannot be divided by 0'))
                self.nextButton.setDisabled(True)
                self.label_function_error.setStyleSheet("QLabel { color : red; }")

    def __disableButtonAndShowMessage(self):
        self.nextButton.setDisabled(True)
        self.statusbar.showMessage("The minimum must be greater than the maximum!")
    ...
    def __printSolution(self):
        ...
        self.solutionBox.append(f'Value:\t{functionValue}')
        for var in variables:
            self.solutionBox.append(var)
        self.solutionBox.append(f'\n\nFound after: {lastBestSolutionIteration} iterations')

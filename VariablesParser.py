...
class VariablesParser:
    ...
    def getVariables(self):
        ...
        if self.__hasNext() and self.__index == 0:
            raise VP_WrongExpression("Unexpected sign: '" + self.__currentChar(1) + "'")
        elif self.__hasNext():
            ind = self.__index
            while True:
                ...
                try:
                    ...
                    err = "Unexpected sign: '" + self.__string[self.__index: ind] + "'"
    ...
    def __parseFunctions(self):
        ...
        if char != 0:
            if not self.__parseBrackets(numOfArgs, char):
                raise Exception("Function parameters " + char + " w brackets")
        else:
            ...
            for i in range(numOfArgs):
                ...
                if i < numOfArgs - 1:
                    if self.__currentChar(1) != ',':
                        raise Exception("Not enough arguments in the function '" + functionName + "'")
                    self.__index += 1
            if self.__currentChar(1) != ')':
    ...
    def __parseNumber(self):
        ...
        if len(value) == 0:
            if char == '':
                raise Exception("Pattern termination unexpectedly")
            else:
...
def evaluateError(expression, constants={}):
    outputStr, code = "No function was given", 1
    if expression != '':
        outputStr, code = "I am doing", 2
        try:
            ...
            for var in variables:
                ...
                if var != variables[len(variables) - 1]:
                    outStr += ', '
            if outStr != '': outputStr, code =  'Variables detected: ' + outStr, 0
